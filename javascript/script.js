  ...
  draw() {
    ...
    for (var i = 0; i < this.waves; i++) {
      ...
      for (var j = 0; j < this.points; j++) {
        ...
        var y =
          Math.sin((x / this.canvas.width) * Math.PI * 2 + this.phase) *
            this.amplitude +
    ...
    for (var j = 0; j < this.points; j++) {
      var x = (j / this.points) * this.canvas.width;
      var y =
        Math.sin((x / this.canvas.width) * Math.PI * 2 + this.phase + Math.PI) *
          (this.amplitude / 2) +
        this.canvas.height / 2; // Different phase and amplitude
      this.ctx.lineTo(x, y);
    }
